...
#[cfg(windows)]
fn connect(paths: &Paths, launch_if_needed: bool) -> Result<std::net::TcpStream, String> {
    use std::net::TcpStream;
    let port_file = paths.data_dir.join("app.port");
    let mut launch = Some(|| {
        let exe = std::env::current_exe().ok().and_then(|p| p.parent().map(|d| d.join("AgentPulse.exe")))
            .ok_or_else(|| "无法定位 AgentPulse.exe".to_string())?;
        std::process::Command::new(exe).arg(BACKGROUND_ARG).stdin(Stdio::null()).stdout(Stdio::null()).stderr(Stdio::null())
            .spawn().map(|_| ()).map_err(|e| format!("无法启动 AgentPulse 后台通知进程：{e}"))
    });
    let lock_path = paths.data_dir.join("app-launch.lock");
    let deadline = Instant::now() + Duration::from_secs(6);
    loop {
        if let Ok(port) = std::fs::read_to_string(&port_file).ok().and_then(|s| s.trim().parse::<u16>().ok()).ok_or(()) {
            if let Ok(stream) = TcpStream::connect(("127.0.0.1", port)) { return Ok(stream); }
        }
        if !launch_if_needed || Instant::now() >= deadline { break; }
        if let Some(_lock) = FileLock::try_acquire(&lock_path).map_err(|e| e.to_string())? {
            if let Some(start) = launch.take() { start()?; }
        }
        std::thread::sleep(Duration::from_millis(300));
    }
    Err("RuntimeError: AgentPulse 没有运行，无法弹窗".into())
}
...
/// 仅投递活动状态，既不启动 App，也不等待回复。
pub fn send_activity(paths: &Paths, req: &Value) {
    if let Ok(mut stream) = connect(paths, false) {
        if stream.set_write_timeout(Some(Duration::from_millis(200))).is_ok() {
