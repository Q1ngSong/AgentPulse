...
pub fn record_activity(paths: &Paths, agent: &str, host_bundle: &str, payload: &Value, kind: &str) -> io::Result<()> {
    ...
    let rec = json!({
        ...
        "tool_use_id": super::events::tool_use_id(payload),
        "tool_input_key": super::events::tool_input_key(payload),
        "hook_event": payload.get("hook_event_name").cloned().unwrap_or(Value::Null),
    });
