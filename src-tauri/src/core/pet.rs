...
pub fn publish(paths: &super::paths::Paths, update: &Update) {
    ...
    {
        use std::io::Write;
        if let Ok(mut socket) = std::os::unix::net::UnixStream::connect(paths.data_dir.join("app.sock")) {
            let _ = socket.set_write_timeout(Some(std::time::Duration::from_millis(200)));
            let _ = writeln!(socket, "{}", serde_json::json!({"kind": "pet_update", "update": update}));
        }
    }
    #[cfg(not(unix))]
    let _ = (paths, update);
