...
pub fn hook_command(binary: &Path, agent: &str) -> io::Result<String> {
    ...
    #[cfg(windows)]
    return Ok(format!("\"{}\" {agent}", path.replace('"', "\\\"")));
    #[cfg(not(windows))]
...
pub fn install(paths: &Paths, home: &Path, agent: &str, hook_command: &str) -> io::Result<Status> {
    ...
    for (_, hook_ev) in HOOK_EVENTS.iter().chain(ACTIVITY_HOOKS.iter()) {
        ...
        // Codex（当前发行版）还不支持 async hook，会直接整条跳过，因此 Codex 用同步 + 15 秒超时兜底。
        let entry = if agent == "codex" {
            json!({"hooks": [{"type": "command", "command": hook_command, "timeout": 15}]})
        } else {
            json!({"hooks": [{"type": "command", "command": hook_command, "async": true, "timeout": 15}]})
        };
...
mod tests {
    ...
    #[test]
    fn codex_hooks_are_synchronous_with_timeout() {
        // 当前发行版 Codex 会整条跳过 async hook（在 hooks/list 里直接消失），
        // 所以 Codex 的条目不能带 "async"，否则永远接不上。
        let (_d, p, home) = setup();
        install(&p, &home, "codex", CMD).unwrap();
