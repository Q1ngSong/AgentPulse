...
pub fn shutdown(paths: &Paths) {
    ...
    #[cfg(not(unix))]
    {
        #[cfg(windows)]
        {
            ...
            let own = OWN_PORT.load(Ordering::Relaxed).to_string();
            if own != "0" && std::fs::read_to_string(&port_file).ok().as_deref() == Some(&own) {
...
#[cfg(windows)]
static OWN_PORT: AtomicU16 = AtomicU16::new(0);

#[cfg(windows)]
fn handle_tcp_conn(app: AppHandle, mut stream: TcpStream) {
    use std::io::{BufRead, BufReader, Write};
    let mut line = String::new();
    if BufReader::new(&mut stream).read_line(&mut line).is_err() { return; }
    let req = match serde_json::from_str::<Value>(line.trim()) {
        Ok(req) => req,
        Err(e) => { let _ = writeln!(stream, "{}", json!({"ok": false, "error": format!("请求不是 JSON：{e}")})); return; }
    };
...
#[cfg(windows)]
fn serve_tcp(app: AppHandle, paths: Paths) {
    let listener = loop {
        match TcpListener::bind("127.0.0.1:0") {
            Ok(listener) => break listener,
            Err(e) => { crate::app_log(&format!("TCP IPC 监听失败：{e}，稍后重试")); std::thread::sleep(Duration::from_secs(1)); }
        }
    };
    let port = listener.local_addr().map(|a| a.port()).unwrap_or(0);
    if port == 0 { return; }
    let _ = std::fs::create_dir_all(&paths.data_dir);
    if std::fs::write(paths.data_dir.join("app.port"), port.to_string()).is_err() { return; }
    OWN_PORT.store(port, Ordering::Relaxed);
    for stream in listener.incoming() {
        match stream {
            Ok(stream) => { let app = app.clone(); std::thread::spawn(move || handle_tcp_conn(app, stream)); }
            Err(e) => crate::app_log(&format!("TCP IPC 接收失败：{e}")),
        }
    }
}
...
pub fn start(app: AppHandle, paths: &Paths) {
    ...
    {
        #[cfg(windows)]
        { let paths = paths.clone(); std::thread::spawn(move || serve_tcp(app, paths)); }
        #[cfg(not(windows))]
        let _ = (app, paths);
    }
}
