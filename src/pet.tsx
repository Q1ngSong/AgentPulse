...
import type { PetFrame, PetMedia } from "./lib/pet-media";
import "./pet.css";
...
function CatFrame({ frame }: { frame?: PetFrame }) {
  const canvas = useRef<HTMLCanvasElement>(null);
  useEffect(() => {
    const ctx = canvas.current?.getContext("2d"); if (!ctx || !frame) return;
    const { image, x, y, width, height } = frame;
    ctx.clearRect(0, 0, 384, 384);
    const scale = Math.min(384 / width, 384 / height);
    const w = width * scale, h = height * scale;
    ctx.drawImage(image, x, y, width, height, (384 - w) / 2, (384 - h) / 2, w, h);
  }, [frame]);
...
function Pet() {
  ...
  useEffect(()=>{
    ...
    }).catch(e=>{if(alive) setMediaError(`动画素材加载失败：${String(e)}`);});
    return ()=>{alive=false;};
  },[animationSignature, retry]);
  ...
  return <main className={demo ? "demo-page" : "pet-page"}>
    ...
    {state.enabled && <section ref={scene} className="pet-scene" aria-label={petName} data-instance={instance} data-playing={playback.phase} data-frame={playback.frame} data-target={phase}
      ...
      <button className={`pet-character${dragging ? " is-dragging" : ""}`} aria-label={canActivate ? `${petName}：返回当前消息的来源 App` : `${petName}：按住拖动，右键打开菜单`}
        ...
        <CatFrame frame={currentMedia?.images[playback.phase][playback.frame]} />
      </button>
      ...
      {!ready && !mediaError && <div className="pet-error" role="status">正在加载桌宠…首次使用需要下载素材</div>}
      {(mediaError || error) && <div className="pet-error" role="alert">{mediaError || error}{mediaError && <button onClick={() => setRetry(value => value + 1)}>重试</button>}</div>}
    </section>}
