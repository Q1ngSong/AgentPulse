  ...
  return <section className="rounded-xl border bg-card p-5">
    ...
    {(Object.keys(PET_NAMES) as PetPhase[]).map(phase=>{
      const a=value[phase]; return <fieldset key={phase} className="mt-5 border-t pt-4">
        ...
        <div className="mt-1 flex gap-2"><Input id={`${phase}-folder`} value={a.folder} placeholder="默认猫咪（首次使用时下载）" onChange={e=>patch(phase,{folder:e.target.value})}/>
        ...
        <p className="mt-1 text-xs text-muted-foreground">留空使用下载的默认猫咪；自定义目录请填完整路径并检查。{a.frames} 帧。</p>
